...
> Dernière mise à jour : 2026-09-15 (**UI** : QCM sans scroll de page ; nom du PJ au-dessus du Dire/Action)
...
4. **Interface** — accueil créer/rejoindre, page `/salon/[code]`, QR + lien, switch god mode (admin) ; **favicon** dé D20 or (`apps/web/src/app/icon.svg` + `apple-icon.tsx`)
   ...
   - **Onboarding hôte (graine)** : après création/reprise salon, tant que la fiche n'est pas `ready` — `HostSetupWizard` (**étape 1/3**) bloque le chat : `AdminLlmForm` (LLM + **curseur Style du récit MJ**) ; test obligatoire ; puis **QCM de départ** (`CharacterBriefForm`, étape 2/3) ; puis `CharacterCreationWizard` fiche (**étape 3/3**). Joueurs non-hôte : QCM puis fiche (pas d'étape LLM). Pendant l'étape 1, le formulaire LLM du god mode est masqué (évite doublon). QCM : **seul le panneau de choix défile** (`char-brief-choices` + `overscroll-behavior` + `html.rpg-wizard-lock`) — plus de page vide sous le wizard.
...
**État VPS (2026-09-15)** : Docker actif ; **MJ via Ollama sur le VPS** (`LM_STUDIO_BASE_URL=http://127.0.0.1:11434/v1`) — plus de tunnel Mac requis. Dernier déploiement : `2728a37` (QCM sans scroll de page, nom au-dessus du Dire). Health et accueil publics **200**. Déploiement : `bash deploy/push-deploy.sh` (Mac) ou `bash deploy/deploy.sh` (VPS). Public : `https://vps-e09ed6db.vps.ovh.net/rpg-cr/`. Un salon déjà ouvert **n’est pas réécrit** : il faut un **nouveau salon** pour l'Acte I.
